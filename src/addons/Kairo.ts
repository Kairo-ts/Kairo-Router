...
import {
  ...
  type AddonProperty,
} from "./AddonPropertyManager";
import { AddonInitializer } from "./router/init/AddonInitializer";
...
type ScriptEventCommandHandler = (
  data: KairoCommand,
...
export class Kairo {
  ...
  private constructor(private readonly properties: KairoAddonProperties) {
  ...
  private static getInstance(properties?: KairoAddonProperties): Kairo {
    if (!this.instance && properties !== undefined) {
      this.instance = new Kairo(properties);
    }
  ...
  private static async _runScriptEvent(
    data: KairoCommand,
  ): Promise<void | KairoResponse> {
